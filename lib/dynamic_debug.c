...
static char *__skip_spaces(const char *str)
{
	str = skip_spaces(str);
	while (*str == ',')
		str = skip_spaces(++str);
...
static int dynamic_emit_prefix_lookups(struct _ddebug *desc, char *buf)
{
...
static inline char *dynamic_emit_prefix(struct _ddebug *desc, char *buf)
{
	...
	if (flags & _DPRINTK_FLAGS_INCL_LOOKUP)
		pos += dynamic_emit_prefix_lookups(desc, buf);
