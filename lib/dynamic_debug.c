			      ...
			      int selected_class)
{
	...
	if (query->filename) {
		match = match_wildcard(query->filename, desc_filename(dp)) ||
			match_wildcard(query->filename, kbasename(desc_filename(dp))) ||
			match_wildcard(query->filename, trim_prefix(desc_filename(dp)));
	...
	if (query->function) {
		match = match_wildcard(query->function, desc_function(dp));
