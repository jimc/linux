...
static char *show_colon_args(struct _ddebug *ctrl, char *p)
{
...
typedef char* (*show_flag_args_f)(struct _ddebug *, char *);
...
struct flagsbuf { char buf[ARRAY_SIZE(opt_array)+FLAG_COLON_ARG_LEN+1]; };

/* format a string into buf[] which describes the _ddebug's flags */
static char *ddebug_describe_flags(struct _ddebug *ctrl, struct flagsbuf *cb)
{
...
static void update_tr_dst(const struct _ddebug *desc, struct _ddebug *new)
{
	int odst = desc->trace_dst;
	int ndst = new->trace_dst;
...
static int ddebug_change(const struct ddebug_query *query, struct flag_settings *modifiers)
{
	...
	list_for_each_entry(dt, &ddebug_tables, link) {
		...
		for (i = 0; i < dt->num_ddebugs; i++) {
			...
			new.flags = (dp->flags & modifiers->mask)
				| modifiers->flags;
			new.trace_dst = select_tr_dst(modifiers, get_trace_dst(dp));
			if (new.flags == dp->flags)
				continue;
#ifdef CONFIG_JUMP_LABEL
			if (dp->flags & _DPRINTK_FLAGS_ENABLED) {
				if (!(new.flags & _DPRINTK_FLAGS_ENABLED))
					static_branch_disable(&dp->key.dd_key_true);
			} else if (new.flags & _DPRINTK_FLAGS_ENABLED) {
			...
			v4pr_info("changed %s:%d [%s]%s %s => %s\n",
				  ...
				  ddebug_describe_flags(dp, &fbuf),
				  ddebug_describe_flags(&new, &nbuf));
			dp->flags = new.flags;
			update_tr_dst(dp, &new);
		}
	}
...
/*
 * handle multiple queries in query string, continue on errors except
 * on open and close commands, return last error or number of matching
 * callsites.  Module name is either in param (for boot arg) or
 * perhaps in query string.
 */
