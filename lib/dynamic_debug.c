 ...
 * special treatment.  Later we could give modules the choice to
 * protect their classes or to keep v2 behavior.
 */
#define ddebug_module_protects_classes(dt) false
/*
	...
	list_for_each_entry(dt, &ddebug_tables, link) {
		...
		for (i = 0; i < dt->info.descs.len; i++) {
			...
			if (dp->class_id != slctd_class) {
				if (query->class_string)
					/* mismatch on given class */
					continue;
				else if (ddebug_module_protects_classes(dt))
...
static void ddebug_condense_sites(struct _ddebug_info *di)
{
	...
	cur_dd = func_dd = file_dd = mod_dd = di->descs.start;
	cur_ds = func_ds = file_ds = mod_ds = di->sites.start;
	i = 0;
	for (; i < di->descs.len; i++, cur_dd++, cur_ds++) {
...
static int ddebug_remove_module(const char *mod_name)
{
	...
	list_for_each_entry_safe(dt, nextdt, &ddebug_tables, link) {
		if (dt->mod_name == mod_name) {
			ddebug_clear_range(dt->info.descs.start, dt->info.descs.len);
