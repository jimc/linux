					    ...
					    gfp_t gfp)
{
	...
	if (bt->node_count < BONSAI_NODES_PER_SLAB) {
		if (bt->node_count == 0) {
			...
			u8 new_cap = min_t(unsigned int, BONSAI_NODES_PER_SLAB,
					   bt->slab0_cap_nodes * 2);
		...
		node = (union bonsai_node *)((char *)bt->node_slabs[0] +
					     bt->node_count * BONSAI_NODE_SIZE);
...
void bonsai_seal(struct bonsai_tree *bt)
{
	...
	if (bt->num_node_slabs == 1 && bt->node_count < bt->slab0_cap_nodes &&
	    bt->node_count > 0) {
		void *tight = krealloc(bt->node_slabs[0],
				       bt->node_count * BONSAI_NODE_SIZE,
				       GFP_KERNEL);
...
int bonsai_invalidate(struct bonsai_tree *bt, unsigned long index)
{
	union bonsai_node *curr;
	u16 curr_idx;
	int slot;
	...
	if (WARN_ON_ONCE(bt->is_sealed))
		return -EPERM;

	if (likely(bt->height == 1)) {
		curr = (union bonsai_node *)bt->node_slabs[0];
		if (likely(curr && curr->m.node_type == BONSAI_TYPE_LEAF)) {
			slot = leaf_find_slot(&curr->l, index);
...
EXPORT_SYMBOL_GPL(bonsai_invalidate);
		       ...
		       unsigned long last, void *val, gfp_t gfp)
{
	...
	if (depth == 0) {
		...
		new_root = bonsai_alloc_node(bt, BONSAI_TYPE_BRANCH_16, gfp);
		if (!new_root)
			return -ENOMEM;
	...
	for (d = depth - 1; d >= 0; d--) {
		...
		if (node->b16.meta.num_pivots < BONSAI_BRANCH_SLOTS_16 - 1) {
		...
		u16 new_branch_idx;
		int old_pivots = node->b16.meta.num_pivots;
		int mid = old_pivots / 2;
		unsigned long sep_pivot;

		new_branch = bonsai_alloc_node(bt, BONSAI_TYPE_BRANCH_16, gfp);
		if (!new_branch)
			return -ENOMEM;
		...
		if (slot == old_pivots) {
			...
			for (i = mid + 1; i <= old_pivots; i++) {
				new_branch->b16.child_idx[i - (mid + 1)] =
					node->b16.child_idx[i];
			...
			if (slot <= mid) {
				...
				for (i = new_branch->b16.meta.num_pivots; i > b_slot; i--) {
					new_branch->b16.pivot[i] =
						new_branch->b16.pivot[i - 1];
					new_branch->b16.child_idx[i + 1] =
						new_branch->b16.child_idx[i];
				}
