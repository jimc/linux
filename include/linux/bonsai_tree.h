...
#define BONSAI_BRANCH_SLOTS	25	/* 24 pivots + 25 u16 child indices */
#define BONSAI_LEAF_SLOTS	16	/* 15 pivots + 16 64-bit value slots */
...
struct bonsai_meta {
	...
	u8 node_type;		/* Offset 3 (BONSAI_TYPE_LEAF / BONSAI_TYPE_BRANCH) */
	u8 num_pivots;		/* Offset 4 */
	u8 _pad[3];		/* Offset 5..7 -> Total: 8 bytes */
};

/*
 * Compact 256-byte branch node (4 cachelines).
 * Uses 16-bit indices into the pot for 25-way fanout.
 */
struct bonsai_branch {
	struct bonsai_meta meta;			/* 8 bytes (0..7) */
	unsigned long pivot[BONSAI_BRANCH_SLOTS - 1];	/* 192 bytes (8..199) */
	u16 child_idx[BONSAI_BRANCH_SLOTS];		/* 50 bytes (200..249) */
...
#define BONSAI_TREE_INIT { NULL, 0, 0, 0, 0, 0 }
