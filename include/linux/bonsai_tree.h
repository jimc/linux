 ...
 * Up to 128 slabs = 2048 nodes (512 KiB capacity)
 */
...
#define BONSAI_MAX_NODE_SLABS	128
 ...
 */
struct bonsai_tree {
	/* Array of node slabs (slab0 dynamically doubles up to 4 KiB) */
	void *node_slabs[BONSAI_MAX_NODE_SLABS];
	struct bonsai_val_slab *val_slabs;      /* Linked value/string slabs */
	u8 num_node_slabs;                      /* Active node slabs count */
	u8 num_val_slabs;                       /* Active value slabs count */
	u8 slab0_cap_nodes;                     /* Cap of slab0 (1..16) */
	u8 height;                              /* 0=empty, 1=leaf, 2=branch */
	u16 root_idx;                           /* 1-based root index */
	bool is_u16;                            /* False=u8, True=u16 */
	bool is_sealed;                         /* True=frozen/read-only */
	u32 val_bytes_used;                     /* Total bytes in value slabs */
	unsigned int node_count;                /* Total active nodes */
	unsigned long entry_count;              /* Total range entries */
};
...
static inline u16 bonsai_node_idx(const struct bonsai_tree *bt, const union bonsai_node *node)
{
	...
	for (slab = 0; slab < bt->num_node_slabs; slab++) {
		...
		slab_size = (slab == 0)
			? ((size_t)bt->slab0_cap_nodes * BONSAI_NODE_SIZE)
			: BONSAI_SLAB_SIZE;
