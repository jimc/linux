...
static struct ledger lockdep_pool =
	LEDGER_INIT(lockdep_pool, list_entries,
		    SCRATCH_16K_ORDER, NULL);

void lockdep_pool_stats(unsigned int *nr_chunks, size_t *chunk_size, size_t *tail_used)
{
	__ledger_stats(&lockdep_pool, nr_chunks, chunk_size, tail_used);
}
...
DEFINE_SIMPLE_2D_RADIX(lock_class, struct lock_class, MAX_LOCKDEP_KEYS, NULL);

void lockdep_class_stats(unsigned int *nr_chunks, size_t *chunk_size, size_t *tail_used)
{
	*nr_chunks = nr_lock_class_chunks;
	*chunk_size = (size_t)nr_lock_class_chunks * (sizeof(struct lock_class) * lock_class_PER_CHUNK);
	*tail_used = (nr_lock_classes & lock_class_CHUNK_MASK) * sizeof(struct lock_class);
}
...
static struct scratchpad lockdep_trace_pool =
	SCRATCHPAD_INIT(lockdep_trace_pool, stack_trace,
			SCRATCH_16K_ORDER, NULL);
...
{
	__scratchpad_stats(&lockdep_trace_pool, nr_chunks, chunk_size, tail_used);
}

static inline void lock_trace_discard(struct lock_trace *trace, unsigned int max_entries)
{
	size_t size = sizeof(struct lock_trace) + max_entries * sizeof(unsigned long);

	__scratchpad_discard(&lockdep_trace_pool, trace, size);
}

static inline void lock_trace_trim(struct lock_trace *trace, unsigned int unused_entries)
{
	__scratchpad_trim(&lockdep_trace_pool, unused_entries * sizeof(unsigned long));
}
...
static struct lock_trace *save_trace(void)
{
	...
	u32 hash;
	size_t size = sizeof(struct lock_trace) + MAX_LOCKDEP_TRACE_DEPTH * sizeof(unsigned long);

	BUILD_BUG_ON_NOT_POWER_OF_2(STACK_TRACE_HASH_SIZE);

	trace = __scratchpad_alloc(&lockdep_trace_pool, size, sizeof(unsigned long), GFP_ATOMIC);
	if (!trace) {
...
static struct lock_list *alloc_list_entry(void)
{
	...
	entry = __ledger_alloc_type(&lockdep_pool, struct lock_list, GFP_ATOMIC);
	if (likely(entry)) {
		nr_list_entries++;
		return entry;
	}
...
DEFINE_SIMPLE_2D_RADIX(lock_chain, struct lock_chain, MAX_LOCKDEP_CHAINS, NULL);
static DECLARE_BITMAP(lock_chains_in_use, MAX_LOCKDEP_CHAINS);

void lockdep_chain_stats(unsigned int *nr_chunks, size_t *chunk_size, size_t *tail_used)
{
	*nr_chunks = nr_lock_chain_chunks;
	*chunk_size = (size_t)nr_lock_chain_chunks * (sizeof(struct lock_chain) * lock_chain_PER_CHUNK);
	*tail_used = 0;
}

DEFINE_SIMPLE_2D_RADIX(chain_hlock, u16, MAX_LOCKDEP_CHAIN_HLOCKS, NULL);
...
{
	*nr_chunks = nr_chain_hlock_chunks;
	*chunk_size = (size_t)nr_chain_hlock_chunks * (sizeof(u16) * chain_hlock_PER_CHUNK);
	*tail_used = chain_hlocks_used() * sizeof(u16);
}
	...
	list_for_each_entry_safe(entry, tmp, &class->locks_after, entry) {
		list_for_each_entry_safe(other, other_tmp, &entry->links_to->locks_before, entry) {
			if (other->links_to == class) {
				...
				__ledger_put(&lockdep_pool, other);
				break;
			}
		}
		nr_list_entries--;
		list_del_rcu(&entry->entry);
		__ledger_put(&lockdep_pool, entry);
	}
...
{
	pr_info("lockdep: %lu/%lu bootstrap entries, %zu/%lu bootstrap traces used before buddy init, scratchrec active\n",
		min_t(unsigned long, nr_list_entries, BOOTSTRAP_LOCKDEP_ENTRIES),
		BOOTSTRAP_LOCKDEP_ENTRIES,
		sizeof(stack_trace) - lockdep_trace_pool.remaining,
