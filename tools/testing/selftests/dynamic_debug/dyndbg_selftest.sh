...
function check_default_dst() {
    dst=$(tail -50 /proc/dynamic_debug/control | grep "#: Default trace destination" | cut -d':' -f3 | sed -e 's/^[[:space:]]*//')
    if [ "$dst" != "$1" ]; then
        echo -e "${RED}: $BASH_SOURCE:$BASH_LINENO default destination '$dst' does not match with '$1'"
        exit
    fi
}
...
function test_default_destination {
    ...
    ddcmd class,D2_CORE,+T      # default dest is 0 but since callsite was already labelled then reuse label
    check_match_ct =T:foo 1 -v
    ...
    ddcmd class,D2_KMS,+T	# default dest is 0 but since callsite was already labelled then reuse label
    ...
    check_default_dst 0		# after closing foo which was default dest we revert to 0 as default dest
...
function test_private_trace_syntax_rmmod {
    ...
    rmmod test_dynamic_debug	# remove module whose callsites are writing debug logs to trace instance foo
    ddcmd "close bar;close foo"	# close bar and foo trace instances, if usage count of foo instance was not
				# correctly decremented during removal of module then close will fail
    ...
    del_trace_instance_dir foo 1	# delete trace instance foo and verify its directory was removed
    check_trace_instance_dir foo 0

    search_trace "test_private_trace_syntax_rmmod about to do_prints"

    del_trace_instance_dir bar 1	# delete trace instance bar and verify its directory was removed
    check_trace_instance_dir bar 0
}

# test verifies new syntax and combination of delete attempt of trace instance which is still in use with module removal
function test_private_trace_syntax_delete_in_use_rmmod {
    ...
    ddcmd "close foo"			# close will succeed only if foo usage count was correctly decremented during module removal
    is_trace_instance_closed foo	# verify trace instance foo is closed
...
# prepares dynamic debug and trace enviroment for tests execution
function setup_env_for_tests {
    echo -e "${GREEN}# SETUP_ENV_FOR_TESTS ${NC}"

    echo "MODULES"
    ifrmmod test_dynamic_debug_submod -v	# unload test_dynamic_debug_submod module if it is loaded
    ...
    cat /proc/dynamic_debug/control | grep -v "=:0" | grep -v "not set" | grep -v "^$" | grep -v "#: Opened trace instances" | grep -v "#: Default trace destination"
    ddcmd module,*,=:0 # clear all flags and set dest to 0
    echo

    # close all opened trace instances and delete their respective directories
    echo "OPEN trace instance"
    output=$(tail -n9 /proc/dynamic_debug/control | grep "#: Opened trace instances" | cut -f3 -d":" | xargs -n1)
    for dst in $output
    do
...
function test_labelling {
    ...
    search_trace_name param_log 5 "params:parse_args:kernel/params.c:178: doing test_dynamic_debug"
    ...
    check_match_ct =T:new_out 4		# the module params prdbgs got moved
    check_match_ct =T:param_log 2	# CORE, KMS remain
    ...
    check_match_ct :param_log 1 -v	# pick up the D2_ATOMIC
    ddcmd label param_log class D2_ATOMIC +:0
    ddcmd close param_log		# now it closes wo -EBUSY

    ifrmmod test_dynamic_debug

    # leave the instances around for inspection ??
    del_trace_instance_dir param_log 0
    del_trace_instance_dir new_out 0
}
