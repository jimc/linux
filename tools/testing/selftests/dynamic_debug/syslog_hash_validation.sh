...
function verify_fingerprint {
    ...
    # Strictly verify that the computed fingerprint matches the expected hash for this label
    if [ -n "$expected_hash" ] && [ "$expected_hash" = "$fingerprint" ]; then
        local short_hash="${fingerprint:0:12}"
        [ "$V" -ge 1 ] && echo -e "${GREEN}✔ Verified '${label}' (${short_hash}) [via: '${LOCAL_DDCMDS}']${NC}"
        ...
        if [ "$status_str" = "DRIFTED" ]; then
            printf "#K= %-32s %-24s %s\n" "${fingerprint}" "${label}" "${extra_args}" >> "$DRIFT_HASHES_FILE"
        else
            printf "#K= %-32s %-24s %s\n" "${fingerprint}" "${label}" "${extra_args}" >> "$UNREG_HASHES_FILE"
        fi
    fi
}
...
function verify_dmesg_slice {
    ...
    if [ "$app" = "DYNDBG" ]; then
        default_filter="dyndbg:|test_dd:|test_dynamic_debug|handling "
    fi
...
function audit_golden_records {
    ...
    while read -r line; do
        ...
        if ! grep -q "$hash" "$seen_file" 2>/dev/null; then
            if [ $stale_found -eq 0 ]; then
                ...
                [ "${V:-0}" -eq 0 ] && echo -e "${YELLOW}# --- GOLDEN_RECORDS Audit ---${NC}"
    ...
    if [ $stale_found -eq 0 ]; then
        [ "${V:-0}" -ge 1 ] && echo -e "${GREEN}# All $total_records GOLDEN_RECORDS entries were successfully hit!${NC}"
    fi
